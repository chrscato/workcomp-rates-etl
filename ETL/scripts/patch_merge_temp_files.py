#!/usr/bin/env python3
"""
Patch script to merge existing temp files using the corrected SQL
...
def merge_temp_files_patched(output_path: Path, table_name: str, keys: list, db_conn) -> None:
    """Merge all temporary files into final output file using corrected SQL"""
    ...
    print(f"Merging {len(temp_files)} temp files for {table_name}")
    ...
    try:
        if len(temp_files) == 1:
            ...
            if table_name == "fact_rate":
                ...
                # For dimensions, deduplicate based on keys using window function
                partition_cols = ", ".join(keys) if keys else ""
                # Use a simple ordering since rowid doesn't exist in parquet files
...
def main():
    """Main entry point"""
    if len(sys.argv) != 2:
        print("Usage: python patch_merge_temp_files.py <payer>")
        print("Example: python patch_merge_temp_files.py aetna")
        sys.exit(1)
    ...
    print(f"🔧 Patching merge for payer: {payer}")
    print("=" * 50)
    ...
    try:
        ...
        for table_name, output_path in output_files.items():
            ...
            merge_temp_files_patched(output_path, table_name, keys, db_conn)
