#!/usr/bin/env python3
"""
Fast NPPES data fetcher using CSV file instead of API calls

This version reads from the NPPES CSV file instead of making API calls,
which is much faster and more reliable. It reads NPIs from the xref file
and looks them up in the CSV data.

Usage:
    python fetch_npi_data_csv.py --threads 5
    python fetch_npi_data_csv.py --threads 10 --limit 1000
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/nppes_csv_fetch.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class NPPESCSVFetcher:
    ...
    def load_csv_data(self):
        """Load the CSV data using memory-efficient approach."""
        logger.info(f"Setting up CSV access from {self.csv_path}")
        ...
        try:
            if self.csv_path.endswith('.zip'):
                ...
                with zipfile.ZipFile(self.csv_path) as z:
                    ...
                    with z.open(csv_filename) as f:
                        ...
                        self.csv_columns = sample_df.columns.tolist()
                        logger.info(f"CSV columns: {len(self.csv_columns)}")
                        
                        # Get total row count by reading in chunks
            ...
            self.csv_total_rows = total_rows
            self.csv_data = None  # We'll read on-demand
            
        except Exception as e:
            logger.error(f"Error setting up CSV access: {e}")
            raise
...
def main():
    parser = argparse.ArgumentParser(description="Fast parallel NPPES data fetcher using CSV")
    ...
    if not args.yes:
        ...
        print(f"📊 CSV contains {len(fetcher.csv_data)} records")
        print()
